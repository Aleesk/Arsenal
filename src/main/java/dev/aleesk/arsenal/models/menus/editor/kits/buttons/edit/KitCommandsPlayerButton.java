...
public class KitCommandsPlayerButton extends Button {
    ...
    public ItemStack getButtonItem(Player player) {
        ...
        if (kit.getCommandsPlayer().isEmpty()) {
            ...
            for (int i = 0; i < kit.getCommandsPlayer().size(); i++) {
                lore.add((menu.selectedPlayerCommand == i ? "&a➨" : "") + " &6" + (i + 1) + ". ▸ &r" + kit.getCommandsPlayer().get(i));
            }
        }
        lore.add("");
        ...
        return new ItemBuilder(XMaterial.COMMAND_BLOCK.parseMaterial())
                .setName("&aPlayer Commands")
                .setLore(lore)
                .build();
    ...
    public void clicked(Player player, int slot, ClickType clickType, int hotbarButton) {
        if (clickType.isLeftClick()) {
            ...
            if (kit.getCommandsPlayer().size() == (menu.selectedPlayerCommand + 1)) {
                menu.selectedPlayerCommand = 0;
            } else {
                ++menu.selectedPlayerCommand;
            }
            ...
            ChatUtil.sendMessage(player, "&eExample&6: &rspawn");
        } else if (clickType.isCreativeAction()) {
            if (kit.getCommandsPlayer().isEmpty()) return;

            kit.getCommandsPlayer().remove(menu.selectedPlayerCommand);
            kit.save();
